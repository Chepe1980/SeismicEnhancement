...
class SpectralDecomposition:
    ...
    def morlet_wavelet_transform(self, trace, center_freq, cycles=6):
        ...
        t = np.arange(-cycles/center_freq, cycles/center_freq, dt)
        if len(t) % 2 == 0:
            t = t[:-1]  # Ensure odd length
            
        # Morlet wavelet formula
        wavelet = np.exp(2j * np.pi * center_freq * t) * np.exp(-(t**2) * (center_freq**2) / (2 * cycles**2))
        wavelet = wavelet.real  # Take real part
    ...
    def cwt_transform(self, trace, center_freq, scales_factor=1.0):
        ...
        try:
            ...
            # For Morlet wavelet: scale = (center_freq * cycles) / (2 * np.pi * frequency)
            cycles = 6  # Number of cycles for Morlet wavelet
            scale = (center_freq * cycles) / (2 * np.pi * center_freq) * scales_factor
            
            # Generate Morlet wavelet
            wavelet = sp_signal.ricker  # Using Ricker as Morlet alternative
            widths = np.array([scale * 10])  # Scale the width
            
            # Perform CWT
            cwtmatr = sp_signal.cwt(trace, wavelet, widths)
            filtered_trace = cwtmatr[0, :]  # Take the first scale
            
            return filtered_trace
            
        except ImportError:
            st.warning("CWT requires scipy. Using FFT fallback.")
...
def create_rgb_plotly_figure(rgb_data, title):
    ...
    # Ensure data is in the correct format (height, width, 3)
...
def display_spectral_decomposition_tab(enhancer):
    ...
    # Colormap selection for spectral decomposition
        ...
        if st.button("Generate RGB Blend", key="sd_rgb_blend"):
            with st.spinner("Creating RGB frequency blend..."):
                if section_type == "Time Slice":
                    ...
                    display_rgb = rgb_blend.transpose(1, 0, 2)  # Transpose for proper display
                else:  # Crossline
                ...
                # Display RGB blend using the custom function
