...
def display_spectral_decomposition_tab(enhancer):
    ...
    if enhancer.original_data is None:
        st.info("Please load a SEG-Y file in the main tab first.")
        return
    
    # Frequency range selection
    col1, col2, col3 = st.columns(3)
    with col1:
        min_freq = st.slider("Minimum Frequency (Hz)", 5, 50, 10)
    with col2:
        max_freq = st.slider("Maximum Frequency (Hz)", 60, 200, 100)
    with col3:
        num_frequencies = st.slider("Number of Frequencies", 5, 20, 10)
    
    frequencies = np.linspace(min_freq, max_freq, num_frequencies).astype(int)
    
    # Time slice selection
    n_samples = enhancer.original_data.shape[2]
    time_slice = st.slider("Time Slice", 0, n_samples-1, n_samples//2)
    actual_time = enhancer.geometry['samples'][time_slice]
    st.info(f"Selected time: {actual_time} ms")
    
    if st.button("Compute Spectral Decomposition", type="primary"):
    ...
    if 'spectral_analyzer' in st.session_state:
        ...
        # Display frequency slice
        freq_slice = spectral_analyzer.get_frequency_slice(selected_freq_idx, time_slice)
        
        fig = px.imshow(freq_slice, 
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            red_weight = st.slider("Red Weight", 0.0, 2.0, 1.0, 0.1)
        with col2:
            green_weight = st.slider("Green Weight", 0.0, 2.0, 1.0, 0.1)
        with col3:
            blue_weight = st.slider("Blue Weight", 0.0, 2.0, 1.0, 0.1)
        
        if st.button("Generate RGB Blend"):
            with st.spinner("Creating RGB frequency blend..."):
                ...
                # Display RGB blend
                fig_rgb = px.imshow(rgb_blend, 
                                  title=f"RGB Frequency Blend\nLow (R): {frequencies[low_freq_idx]}Hz, "
                ...
                with col1:
                    low_component = spectral_analyzer.get_frequency_slice(low_freq_idx, time_slice)
                    fig_low = px.imshow(low_component, 
                ...
                with col2:
                    mid_component = spectral_analyzer.get_frequency_slice(mid_freq_idx, time_slice)
                    fig_mid = px.imshow(mid_component, 
                ...
                with col3:
                    high_component = spectral_analyzer.get_frequency_slice(high_freq_idx, time_slice)
                    fig_high = px.imshow(high_component, 
